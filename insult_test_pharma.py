...
continuedVer = '3_pharma_stopper'  # Номер набора начальных условий и настроек
...
k2 = decimal.Decimal('2') / 360;  # I->D
k4 = decimal.Decimal('10')/90  # I->F
k4minus = decimal.Decimal('0.001')/90  # F->I
k5 = decimal.Decimal('0.01')/90  # F->H
k7 = decimal.Decimal('1.2') / 90;  # IH->HH
k8 = decimal.Decimal('2') / 90;  # HI->II
k9 = decimal.Decimal('1.2') / 90  # HD->ID
...
# Вычисление R
def get_r_event(R, E=False):
    Ep_minus1 = nol0
    if not E:
        E = decimal.Decimal(str(numpy.random.uniform())) #numpy.random.uniform() numpy.random.random()
...
def get_r_event2(R):
    E = decimal.Decimal(str(numpy.random.random())) #numpy.random.uniform()
    ER = E * R
    ...
    # print (f"R_={speeds_dict[speeds_dict_key]['R_']}, Events++={sum([ev['speed'] for ev in speeds_dict[speeds_dict_key]['events_']])}")
