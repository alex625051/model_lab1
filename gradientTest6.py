...
colorsRed_Green = colorsRed_red+ colorsRed_Ye+colorsY_green
...
def takeRaw(x0):
    ...
    for y0 in np.arange(1, 1000, 1):
        ...
        bufB2=(1000-b2)/2
        xy=Point(x0,y0)
        firstKvPoindo=commonF(xy, (a1/4)-bufA, (b1/4)-bufB)
        firstKvPoint=commonF(xy, a1/4, b1/4)
        firstKvPoinposle=commonF(xy, (a1/4)+bufA, b1/4+bufB)
        secondKvPoinDo=commonF(xy, (a1/1)-bufA, (b1/1)-bufB)
        secondKvPoint=commonF(xy, a1/1, b1/1)
        secondKvPoinPosle=commonF(xy, (a1/1)+bufA, (b1/1)+bufB)
        oversdKvPoin=commonF(xy, 1000, 1000)


        l_0_thePoint=rasst(OO,xy)
        l_0_firstKvPoindo=rasst(OO,firstKvPoindo)
        l_0_firstKvPoint=rasst(OO,firstKvPoint)
        l_0_firstKvPoinposle=rasst(OO,firstKvPoinposle)
        l_0_secondKvPoinDo=rasst(OO,secondKvPoinDo)
        l_0_secondKvPoint=rasst(OO,secondKvPoint)
...
im.save("out2/heatbackground9j_cik.png")
im.show()
